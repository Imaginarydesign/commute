...
$(function(){
  ...
  $('#setPrice').click(function(){
    if($('#fuelPriceInput').val() !== '') {
      localStorage.setItem('fuel', ($('#fuelPriceInput').val() * 100).toFixed(0));
      $('#fuelModal').modal('hide');
      $('#fuelPrice').text('£' + (localStorage.getItem('fuel') / 100).toFixed(2));
    }
  });
  ...
  function calcCost (distanceMiles, timesInput, totalDuration, totalMiles) {
    ...
    if (localStorage.getItem('fuel')) {
      fuelPrice = localStorage.getItem('fuel');
    } else {
      fuelPrice = 110;
    }
